...
// ================= LOGO BASE64 =================
const LOGO_KIRI_BASE64 = "data:image/png;base64,..."; // isi Base64 logo kiri
const LOGO_KANAN_BASE64 = "data:image/png;base64,..."; // isi Base64 logo kanan
...
function login() {
  ...
  fetch(`${API_URL}?action=login&user=${encodeURIComponent(user)}&pass=${encodeURIComponent(pass)}`)
    .then(r=>r.json())
    .then(res=>{
      if(res.status!=="ok"){ alert("❌ Login admin gagal"); document.getElementById("loading").style.display="none"; return; }
...
function loadData() {
  fetch(API_URL+"?action=admin")
    .then(r=>r.json())
    .then(data=>{ allData=data; isiFilterNama(); tampilkan(); document.getElementById("loading").style.display="none"; })
    .catch(()=>{ alert("❌ Gagal memuat data"); document.getElementById("loading").style.display="none"; });
}
...
// ================= REFRESH =================
function refreshData(){ document.getElementById("loading").style.display="block"; loadData(); }
...
// ================= CETAK PDF RESMI =================
function cetakPDF(){
  if(document.querySelectorAll("#data tr").length===0){ alert("❗ Data kosong, tidak bisa dicetak"); return; }
